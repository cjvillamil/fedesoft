...
const appReciclarRouter = express.Router();

appReciclarRouter.use(bodyParser.json());

appReciclarRouter.route('/')
.all((req,res,next) => {
    res.statusCode = 200;
...
appReciclarRouter.route('/:puntoId')
.all((req,res,next) => {
    res.statusCode = 200;
    res.setHeader('Content-Type', 'text/plain');
    next();
...
module.exports = appReciclarRouter;